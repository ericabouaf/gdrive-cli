...
const CONFIG_DIR = join(homedir(), '.config', 'gdrive');
...
/**
 * Load configuration from ~/.config/gdrive/config.json
 */
export function loadConfig() {
  ...
  try {
    ...
    if (!profileConfig.GDRIVE_OAUTH_PATH) {
      throw new Error(`GDRIVE_OAUTH_PATH not found in profile "${currentProfile}"`);
    }
...
export function loadCredentials() {
  ...
  if (!existsSync(config.GDRIVE_OAUTH_PATH)) {
    throw new Error(`OAuth credentials file not found at ${config.GDRIVE_OAUTH_PATH}`);
  }

  try {
    const credentials = JSON.parse(readFileSync(config.GDRIVE_OAUTH_PATH, 'utf8'));
...
export async function getAuthClient() {
  ...
  if (!token) {
    throw new Error('Not authenticated. Please run: gdrive auth login');
  }
  ...
  if (token.expiry_date && token.expiry_date < Date.now()) {
    try {
      ...
      throw new Error('Token expired and refresh failed. Please run: gdrive auth login');
    }
  }
...
export async function login() {
  ...
  const about = await gdrive.about.get({ fields: 'user' });
...
export async function checkStatus() {
  ...
  if (!initialToken) {
    ...
    console.log('Run "gdrive auth login" to authenticate');
    return;
  }
  ...
  try {
    ...
    // Use gdrive API to get user info
    const gdrive = google.drive({ version: 'v3', auth: oauth2Client });
    const about = await gdrive.about.get({ fields: 'user' });
    ...
    console.log('Run "gdrive auth login" to re-authenticate');
  }
}
